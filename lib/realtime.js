...
// braid-text (replaces OT)
const braidServer = require('./braid-server')
...
// Initialize braid-server with references to notes and models
// so it can enforce edit permissions and track authorship on PUT requests
braidServer.init(notes, models)
// update when the note is dirty
setInterval(function () {
  async.each(Object.keys(notes), function (key, callback) {
    const note = notes[key]
    if (braidServer.isDirty(key)) {
      logger.debug(`updater found dirty note: ${key}`)
      updateNote(note, function (err, _note) {
        // handle when note already been clean up
        if (!notes[key]) return callback(null, null)
        if (!_note) {
...
function finishUpdateNote (note, _note, callback) {
  ...
  braidServer.getDocument(note.id).then(function (body) {
...
function finishConnection (socket, noteId, socketId) {
  ...
  // Send document state and braid resource URL to client
  // (replaces OT server's addClient which sent doc + revision)
  braidServer.getDocument(noteId).then(function (body) {
    socket.emit('doc', {
      str: body,
      braidUrl: '/braid/' + noteId
    })
    // Emit name/color for cursor rendering
    socket.broadcast.to(noteId).emit('set_name', socket.id, user.name)
    socket.broadcast.to(noteId).emit('set_color', socket.id, user.color)
  })
...
function startConnection (socket) {
  ...
  if (!notes[noteId]) {
    ...
    models.Note.findOne({
      ...
      // Load document into braid-text (replaces OT server creation)
      braidServer.loadNote(noteId, body).then(function () {
        const authors = {}
...
function disconnect (socket) {
  ...
  if (note) {
    ...
    if (Object.keys(note.users).length <= 0) {
      if (braidServer.isDirty(noteId)) {
        updateNote(note, function (err, _note) {
          if (err) return logger.error('disconnect note failed: ' + err)
          // unload from braid-text to free memory
          braidServer.unloadNote(noteId)
          delete notes[noteId]
          if (config.debug) {
            logger.debug(notes)
            getStatus(function (data) {
...
// Permission checking and authorship tracking are now handled in braid-server.js:
// - checkEditPermission() enforces note.permission on PUT requests (was ifMayEdit)
// - trackAuthorship() records author + lastchangeuser on successful PUTs (was operationCallback)

function updateHistory (userId, note, time) {
  const noteId = note.alias ? note.alias : models.Note.encodeNoteId(note.id)
  braidServer.getDocument(note.id).then(function (body) {
    history.updateHistory(userId, noteId, body, time)
  }).catch(function (err) {
    logger.error('update history failed: ' + err)
  })
}
